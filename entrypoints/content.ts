...
const colorizeManaba = () => {
  ...
  Array.from(titleRow.children).forEach((element, idx) => {
    ...
    const matchedKey = Object.keys(COLUMN_TITLES).find(key => COLUMN_TITLES[key as availableColumnTitles] === element.textContent);
  ...
  rows.forEach((row) => {
    ...
    if (column_indexes.STATUS) {
      ...
      if (statusText === STATUS_TEXTS.notSubmitted || statusText === STATUS_TEXTS.closed) {
        return;
      }
    }

    const deadlineText = column_indexes.ENDS_AT ? columns[column_indexes.ENDS_AT]?.textContent : undefined;
    ...
    daysLeftColumn.textContent = `約${formatDistanceToNowStrict(deadline, { locale: ja, addSuffix: true })}`;
